...
const seed = async () => { // make a bunch of users
    let users = []
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        otherUser.forEach(user => user.followercount ++)
        users[i].following = otherUser.map(user => {
    ...
    for (let i = 0; i < 200; i += 1) {
        ...
        let newtweet = {
            ...
            likes: _.sampleSize(users, length).map(user => {
